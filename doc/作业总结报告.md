...
**作者**: Shou Yichen  
**日期**: 2026-08-05
**技术栈**: FastAPI + Tesseract (pytesseract) + OpenAI LLM + OpenCV
**测试环境**: macOS 26.6 (arm64)，Apple M1 Pro（8 核）；Python 3.9.6（项目 `.venv`）；`pytest` 8.4.2；Tesseract 5.5.3（`/opt/homebrew/bin/tesseract`）；自动化测试：5 个通过，5 条警告
...
graph TB
    ...
    subgraph OCR_Pipeline["OCR 处理管线"]
        ...
        D[OCR 引擎<br/>Tesseract (pytesseract)<br/>system binary]
        E[文本标准化<br/>空白字符清理]
    end
...
**耗时分析**: OCR（Tesseract/pytesseract CPU 模式）约 3–7 秒。LLM 推理约 15–21 秒（含远程 API 网络延迟）。端到端总耗时约 21–26 秒/份文档。

> **测试环境**: Intel(R) Core(TM) i7-8700 CPU @ 3.20GHz, Ubuntu 22.04, 32GB RAM, 无独立显卡。Tesseract 运行在纯 CPU 模式下。
